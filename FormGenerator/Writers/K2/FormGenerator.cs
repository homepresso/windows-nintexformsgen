...
namespace K2SmartObjectGenerator
{
    ...
    public class FormGenerator
    {
        ...
        public FormGenerator(ServerConnectionManager connectionManager, string formTheme = "Lithium", SmartObjectGenerator smoGenerator = null, GeneratorConfiguration config = null)
        {
        ...
        public void GenerateFormsFromJson(string jsonContent, Dictionary<string, string> viewTitles = null)
        {
            ...
            string formCategory = $"{_config.Form.TargetFolder}\\{baseFormDisplayName}";
            Console.WriteLine($"[DEBUG] Target Folder from config: {_config.Form.TargetFolder}");
            Console.WriteLine($"[DEBUG] Form Category Path will be: {formCategory}");

            // IMPORTANT: Ensure the category exists BEFORE deploying any forms
            // This prevents timing issues where forms are deployed but not immediately visible
            ...
            foreach (JObject viewDef in viewsArray)
            {
                ...
                FormDefinition formDef = new FormDefinition
                {
                    ...
                    DisplayName = baseFormDisplayName,
    ...
    string areaItemName, string visibility)
            ...
            using (FormsManager formsManager = new FormsManager())
            {
                try
                {
                    ...
                    using (AutoGenerator autoGenerator = new AutoGenerator(formsManager.Connection))
                    {
                        ...
                        // Update form display name to use spaces instead of underscores
                        UpdateFormDisplayName(formDoc, formDef.DisplayName);
                        ...
                        Console.WriteLine($"[DEBUG] About to deploy form to category: {formDef.CategoryPath}");
                        formsManager.DeployForms(generatedForm.ToXml(), formDef.CategoryPath, true);

                        // Add delay to allow K2 to fully register the form
                        Console.WriteLine($"    Waiting for K2 to register form...");
        ...
        private void EnsureCategoryExists(string categoryPath)
        {
            Console.WriteLine($"[DEBUG] Pre-creating category: {categoryPath}");
            ...
            using (FormsManager formsManager = new FormsManager())
            {
                try
                {
                    ...
                    // Create a minimal placeholder form to ensure the category exists
                    // This is a workaround because K2 API doesn't have a direct CreateCategory method
                    string dummyFormXml = @"<?xml version=""1.0"" encoding=""utf-8""?>
<Form xmlns=""http://schemas.k2.com/smartforms/2016/06/xmlschema""
      Name=""_CategorySetup""
      DisplayName=""_CategorySetup"">
</Form>";

                    try
                    {
                        // Deploy the dummy form to create the category
                        formsManager.DeployForms(dummyFormXml, categoryPath, true);
                        Console.WriteLine($"[DEBUG] Category established via placeholder form");

                        // Immediately delete the placeholder form
                        System.Threading.Thread.Sleep(1000); // 1 second to ensure K2 processes the deployment

                        try
                        {
                            formsManager.DeleteForm("_CategorySetup");
                            Console.WriteLine($"[DEBUG] Placeholder form removed, category ready");
                        }
                        catch
                        {
                            // Form might not exist, that's okay
                            Console.WriteLine($"[DEBUG] Placeholder already cleaned up");
                        }

                        // Add additional delay to ensure K2 indexes the category
                        System.Threading.Thread.Sleep(500);
                    }
                    catch (Exception ex)
                    {
                        // Category might already exist with forms, which is fine
                        Console.WriteLine($"[DEBUG] Category setup note: {ex.Message}");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[DEBUG] Category pre-creation note: {ex.Message}");
                    // Don't throw - let the form deployment handle category creation if needed
                }
            }
        }

        private void UpdateFormDisplayName(XmlDocument doc, string displayName)
        {
            // Find the Form element and update its DisplayName attribute
            XmlNodeList formElements = doc.GetElementsByTagName("Form");
            if (formElements.Count > 0)
            {
                XmlElement formElement = (XmlElement)formElements[0];

                // The form display name should use spaces instead of underscores
                formElement.SetAttribute("DisplayName", displayName);
                Console.WriteLine($"    Updated form display name to: {displayName}");
            }
        }
